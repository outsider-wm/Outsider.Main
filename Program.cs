    ...
    static async Task Main()
    {
        ...
        windows.ForEach(win =>
        {
            if (IsCommon(win))
            {
                ...
                Console.WriteLine(WrappedWin32UserApi.GetMonitorInfo(win.MonitorHandle).WorkArea.ToString());
            }
        });

        canTiledWindows.ForEach(win =>
        {
            //WrappedWin32UserApi.ShowWindow(win.Handle, NativeWin32UserApiStructs.ShowWindowMode.SW_NORMAL);
            //WrappedWin32UserApi.MoveWindow(win.Handle, 0, 0, 200, 200, true);
        });
    }
}